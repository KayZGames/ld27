...
class BulletSpawningSystem extends EntityProcessingSystem {
  ...
  BulletSpawningSystem()
      : super(Aspect.getAspectForAllOf([Gun, Transform]).exclude([Cooldown]));

  void initialize() {
    gm = new Mapper<Gun>(Gun, world);
    tm = new Mapper<Transform>(Transform, world);
  }

  void processEntity(Entity entity) {
    var g = gm[entity];
    var pos = tm[entity].position;
    entity.addComponent(new Cooldown(g.cooldown));
    entity.changedInWorld();
    addNewEntity(world, [
      new Transform.w2d(pos.x, pos.y, 0.0),
...
class CollisionImpactSpawningSystem extends EntityProcessingSystem {
  ...
  CollisionImpactSpawningSystem() : super(
          Aspect.getAspectForAllOf([Collision, ImpactOnCollision, Transform]));
  ...
  void processEntity(Entity entity) {
    ...
    addNewEntity(world, [
      new Transform.w2d(pos.x, pos.y, 0.0),
      new BodyDef('impact_${random.nextInt(8)}'),
      new Effect(),
      new ExpirationTimer(50 + random.nextDouble() * 100)
    ]);
    addNewEntity(world, [
      new Transform.w2d(pos.x, pos.y, 0.0),
      new BodyDef('impact_${random.nextInt(8)}'),
...
class DestructionExplosionSpawningSystem extends EntityProcessingSystem {
  ...
  DestructionExplosionSpawningSystem() : super(Aspect
          .getAspectForAllOf([Destruction, ExplosionOnDestruction, Transform]));
  ...
  void processEntity(Entity entity) {
    ...
    addNewEntity(world, [
      new Transform.w2d(pos.x, pos.y, 0.0),
      new BodyDef('explosion_${random.nextInt(8)}'),
      new Effect(),
      new ExpirationTimer(50 + random.nextDouble() * 100)
    ]);
    addNewEntity(world, [
    ...
    addNewEntity(
        world, [new Sound('explosion_${random.nextInt(SOUNDS_EXPLOSION)}')]);
    addNewEntity(
        world, [new Sound('explosion_${random.nextInt(SOUNDS_EXPLOSION)}')]);
  }
}
...
class TruckSpawningSystem extends IntervalEntitySystem {
  ...
  void processEntities(_) {
    addNewEntity(world, [
      new Transform.w2d(random.nextDouble() * MAX_WIDTH, -20.0, PI),
