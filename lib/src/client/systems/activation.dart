...
class FeatureActivationSystem extends IntervalEntitySystem {
  ...
  FeatureActivationSystem(this.achievementLoader, this.graphicsLoader,
      this.sheet, this.audioManager)
      : super(10000, Aspect.getEmpty());
  ...
  void initialize() {
    ...
    features[nextFeatureIndex].preparationResult =
        features[nextFeatureIndex].prepare();
  }
  ...
  void processEntities(Iterable<Entity> entities) {
    ...
    addNewEntity(world, [
      new Achievement(feature.achivementId, playtime),
      new ExpirationTimer(8000.0)
    ]);
  ...
  Feature get _achivementFeature {
    ...
    var activate = (_) => (world
            .getSystem(AchievementRenderSystem) as AchievementRenderSystem)
        .activate();
  ...
  void _activateGraphicsUpdate(Future<SpriteSheet> layerFuture) {
    layerFuture.then((layer) {
      sheet.add(layer);
      (world.getSystem(BackgroundRenderingSystem) as BackgroundRenderingSystem)
          .prepareBackground(1);
    });
  }
  ...
  Future _prepareSoundActivation() {
    ...
    for (int i = 0; i < SOUNDS_EXPLOSION; i++) {
      futures.add(audioManager
          .makeClip('explosion_$i', 'explosion_$i.$fileExtension')
          .load());
    }
    futures
        .add(audioManager.makeClip('shoot_0', 'shoot_0.$fileExtension').load());
    futures.add(audioManager
        .makeClip('collision_0', 'collision_0.$fileExtension')
        .load());
